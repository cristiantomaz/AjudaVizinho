...
vi.mock('firebase/firestore', () => ({
  collection: vi.fn(), doc: vi.fn(), getDocs: vi.fn(), onSnapshot: vi.fn(), query: vi.fn(),
...
const { createInterest, decideInterest, listInterests, markOwnerInterestsSeen } = await import('../src/services/interestService.js')
...
describe('fluxo de interesses no modo demonstração', () => {
  ...
  it('cria uma solicitação e impede duplicidade', async () => {
    ...
    expect(interest).toMatchObject({ status: 'pending', interestedUserId: 'interessado', ownerSeen: false })
  ...
  it('marca solicitações recebidas como visualizadas sem duplicar notificações', async () => {
    const interest = await createInterest({ id: 'item-1', title: 'Cadeira', ownerId: 'doador', status: 'available' }, { id: 'interessado', name: 'Vizinho' })
    const viewed = await markOwnerInterestsSeen([interest], { id: 'doador' })
    expect(viewed[0].ownerSeen).toBe(true)
    await expect(markOwnerInterestsSeen(viewed, { id: 'doador' })).resolves.toEqual(viewed)
  })
